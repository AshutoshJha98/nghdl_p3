    ...
    def createKicadLibrary(self):
        ...
        if xmlFound is None:
            ...
            ret = QtGui.QMessageBox.critical(
             self, "Critical", '''<b>The Libraries of this model ''' +
             '''already exist.Do you want to overwrite it?</b><br/>
             <b>If yes, press ok else cancel it and''' +
             ''' change the name of your vhdl file</b>''',
            ...
            ret = QtGui.QMessageBox.critical(
             self, "Error", '''<b>A standard library already exists in this name.</b><br/>
             <b>Please change the name of your vhdl file and''' +
             '''upload it again</b>''',
