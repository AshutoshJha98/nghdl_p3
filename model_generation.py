...
# comment
comment_vhdl = "------------------------------------------------------------"
comment_vhdl += "--------------------\n--This testbench has been created by "
comment_vhdl += "Ambikeshwar Srivastava, FOSSEE, IIT Bombay\n--------------"
comment_vhdl += "--------------------------------------------------------\n"
comment_vhdl += "----------------------------------------------------------"
comment_vhdl += "----------------------------\n--Modified by Rahul Paknikar"
comment_vhdl += ", FOSSEE, IIT Bombay\n--retrieves "
comment_vhdl += "the IP-Addr from sock_pkg and forwards it to the ghdlserver\n"
comment_vhdl += "------------------------------------------------------------"
comment_vhdl += "--------------------\n"
...
for item in input_port:
    ...
    process.append('\t\t\tassert false report "Get port value ' +
                   item.split(':')[0] + ' returns " &' + item.split(':')[0] +
                   '_v severity note;\n')
    if port_vector_info[port_vector_count]:
...
for item in output_port:
    ...
    process.append(
                   ...
                   item.split(':')[0] + '_v,' + item.split(':')[1] + ');\n')
    process.append(
                   '\t\t\tassert false report "Set port value ' +
                   item.split(':')[0] + ' returns " &' + item.split(':')[0] +
                   '_v severity note;\n')
    process.append("\n")
